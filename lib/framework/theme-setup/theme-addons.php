...
class GhostPool_Addons_Manager {
	...
	function do_plugin_update( $slug ) {
		...
		$status['error'] = esc_html__( 'The plugin does not have an update.', 'socialize' );
		wp_send_json_error( $status );
	...
	function install_child_theme( $slug ) {
		if ( empty( $this->plugins[ $slug ] ) ) {
			wp_send_json_error( array( 'error' => esc_html__( 'We cannot find this child theme.', 'socialize' ) ) );
		}
		...
		if ( ! current_user_can( 'install_themes' ) ) {
			$status['error'] = esc_html__( 'You do not have the permissions to install themes.', 'socialize' );
		...
		if ( isset( $plugin_config['addon_type'] ) && $plugin_config['addon_type'] === 'child_theme' ) {
			...
			if ( $this->is_child_theme_installed() ) {
				...
				if ( $this->is_child_theme_active() ) {
					...
					$status['status_text'] = esc_html__( 'Active', 'socialize' );
					$status['action_text'] = esc_html__( 'Child theme installed and active', 'socialize' );
					$status['action']      = 'no_action';
				} else  {
					$status['status']      = 'gp-inactive';
					$status['status_text'] = esc_html__( 'Inactive', 'socialize' );
					$status['action_text'] = esc_html__( 'Activate child theme', 'socialize' );
					$status['action']      = 'enable_child_theme';
				}
			} else{
				$status['status']      = 'gp-not-installed';
				$status['status_text'] = esc_html__( 'Not installed', 'socialize' );
				$status['action_text'] = esc_html__( 'Install child theme', 'socialize' );
				$status['action']      = 'install_theme';

				if ( ! current_user_can( 'install_themes' ) ) {
					$status['status']         = 'gp-not-installed gp-addons-disabled';
					$status['action_text']    = esc_html__( 'Permissions needed to install child themes. Contact site administrator.', 'socialize' );
			...
			if ( $this->is_plugin_installed( $plugin_slug ) ) {
				if ( $this->does_plugin_have_update( $plugin_slug ) ) {
					...
					$status['status_text']    = esc_html__( 'Needs update', 'socialize' );
					$status['action_text']    = esc_html__( 'Update plugin', 'socialize' );
					$status['action']         = 'update_plugin';
				}
				elseif ( $this->is_plugin_active( $plugin_slug ) ) {
					$status['status']         = 'gp-active';
					$status['status_text']    = esc_html__( 'Active', 'socialize' );
					$status['action_text']    = esc_html__( 'Deactivate plugin', 'socialize' );
					$status['action']         = 'disable_plugin';
				} else  {
					$status['status']         = 'gp-inactive';
					$status['status_text']    = esc_html__( 'Inactive', 'socialize' );
					$status['action_text']    = esc_html__( 'Activate plugin', 'socialize' );
				...
				$status['status']         = 'gp-not-installed';
				$status['status_text']    = esc_html__( 'Not Installed', 'socialize' );
				$status['action_text']    = esc_html__( 'Install plugin', 'socialize' );
				$status['action']         = 'install_plugin';

				if ( ! current_user_can( 'install_plugins' ) ) {
					$status['status']         = 'gp-not-installed gp-addons-disabled';
					$status['action_text']    = esc_html__( 'You do not have permission to install plugins. Contact the site administrator.', 'socialize' );
					$status['action']         = 'contact_network_admin';
				}
	...
	function do_plugin_install( $slug, $echo = true ) {
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			$status['error'] = esc_html__( 'You do not have permission to install plugins.', 'socialize' );
			if ( $echo ) {
				wp_send_json_error( $status );
			} else {
				return $status;
